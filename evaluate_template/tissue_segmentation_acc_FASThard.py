...
threshold_ls = np.arange(0.1, 0.5, 0.05)
headers = ['SUBID']
...
for s in subjects:
    ...
    fast_seg0 = nib.load(os.path.join(args.seg_dir, s, 'reconall_brain_fast_seg_0.nii.gz'))
    fast_seg1 = nib.load(os.path.join(args.seg_dir, s, 'reconall_brain_fast_seg_1.nii.gz'))
    fast_seg2 = nib.load(os.path.join(args.seg_dir, s, 'reconall_brain_fast_seg_2.nii.gz'))
...
wm_csv = os.path.join(args.out_dir, 'Template_'  + args.age_min + args.age_max + args.race + '_WM_FASThard.csv')
df_wm = pd.DataFrame(data_wm)
df_wm.to_csv(wm_csv, header=headers, index=False)
gm_csv = os.path.join(args.out_dir, 'Template_'  + args.age_min + args.age_max + args.race + '_GM_FASThard.csv')
df_gm = pd.DataFrame(data_gm)
df_gm.to_csv(gm_csv, header=headers, index=False)
csf_csv = os.path.join(args.out_dir, 'Template_'  + args.age_min + args.age_max + args.race + '_CSF_FASThard.csv')
